...
export function StageExplain() {
  const { algorithm, runAccuracy, hypothesis, shots, backend, goToStage, completeStage } = useLab()
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {algorithm.basisStates.map((state) => (
              <div key={state} className="rounded-lg border border-border bg-card/60 p-2.5 text-center">
                ...
                <p className="text-[11px] text-amber-400">Actual {actual[state] ?? 0}%</p>
              </div>
            ))}
          </div>

          <div className="mt-5 rounded-xl border border-amber-400/25 bg-amber-400/5 p-4">
            <p className="text-[10px] font-medium uppercase tracking-wider text-amber-400">Empirical Invariant</p>
