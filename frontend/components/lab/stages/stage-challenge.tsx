...
export function StageChallenge() {
  ...
  return (
    <div>
        ...
        action={
          challengeSubmitted && (
            <button
              type="button"
              onClick={() => goToStage(9)}
              className="flex items-center gap-1.5 rounded-full bg-amber-400 px-4 py-2 text-sm font-semibold text-black transition-colors hover:bg-amber-300"
            >
      ...
      <div className="mb-4 flex items-start gap-2 rounded-lg border border-amber-400/25 bg-amber-400/5 px-3 py-2.5 text-xs text-amber-300">
        ...
        {GATE_PALETTE.map((item) => (
          <button
            ...
            className={cn(
              ...
              selectedGate === item.gate ? "ring-2 ring-amber-400" : "opacity-70 hover:opacity-100",
            )}
          >
            {item.gate}
          </button>
      ...
      {!challengeSubmitted ? (
        <button
          ...
          className="mt-4 w-full rounded-full bg-amber-400 px-4 py-3 text-sm font-semibold text-black transition-colors hover:bg-amber-300"
        >
          Submit for Scoring
        </button>
      ) : (
