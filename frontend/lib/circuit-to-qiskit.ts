...
export function circuitToQiskit(gates: GatePlacement[], qubits: number): string {
  const lines: string[] = ["from qiskit import QuantumCircuit", "", `qc = QuantumCircuit(${qubits}, ${qubits})`, ""]
  ...
  if (sorted.length === 0) {
    ...
    for (const g of sorted) {
      ...
      const comment = g.gate === "CNOT" ? `# t${g.step}: CNOT q[${g.qubit}] → q[${g.target ?? (g.qubit === 0 ? 1 : 0)}]` : `# t${g.step}: ${g.gate} on q[${g.qubit}]`
      lines.push(comment)
      if (g.gate === "CNOT") {
  ...
  lines.push("", `qc.measure([${Array.from({ length: qubits }, (_, i) => i).join(", ")}], [${Array.from({ length: qubits }, (_, i) => i).join(", ")}])`)
