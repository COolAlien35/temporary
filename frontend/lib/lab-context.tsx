...
export function LabProvider({
  ...
  const algorithm = useMemo(() => {
    const legacy = getAlgorithm(algorithmSlug === "grovers" ? "deutsch-jozsa" : algorithmSlug)
    const config = getAlgorithmConfig(algorithmSlug)
    if (!config) return legacy
    return { ...legacy, slug: config.meta.slug, name: config.meta.title, qubits: config.register.qubits, backendName: "Local State-Vector Simulator" }
  }, [algorithmSlug])
