...
export function CurrentModuleCard() {
  return (
    ...
    >
      <TiltCard strength={4}>
        <section className="rounded-2xl border border-white/10 border-l-2 border-l-[#00D4FF] bg-white/[0.03] p-5 sm:p-6">
            ...
            {stages.map((stage, i) => (
              <li key={stage.label} className="flex flex-1 items-center last:flex-none">
                ...
                {i < stages.length - 1 &&
                  (stage.status === "done" ? (
                    <motion.div
                      className="mx-1.5 h-px flex-1 origin-left bg-[#00D4FF]/50"
                      initial={{ scaleX: 0 }}
                      animate={{ scaleX: 1 }}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            <motion.button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[#00D4FF] to-[#4FD1E8] px-5 py-2.5 text-xs font-semibold text-[#0A0E17] shadow-[0_0_25px_-5px_rgba(0,212,255,0.6)] transition-shadow hover:shadow-[0_0_35px_-5px_rgba(0,212,255,0.9)]"
            >
              Continue lab
            ...
            <a href="#" className="text-xs font-medium text-white/60 underline-offset-4 hover:text-white hover:underline">
              Open notebook
            </a>
